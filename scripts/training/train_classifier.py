...
from utils.metrics import classifier_metrics
...
def encode_dataset(ds, encoder, device, batch_size=1024):
    X, y = ds.tensors
    out = [
        encoder(X[i : i + batch_size].to(device)).cpu()
...
def evaluate(model, loader, *, criterion, device):
    ...
    return {"loss": loss, **classifier_metrics(probs, targets)}
...
def create_parser():
    parser = argparse.ArgumentParser(
        description="Train MLP classifier on embeddings with k-fold CV."
    )
    parser.add_argument(
        "--input-path",
        required=True,
...
def train_single_fold(fold_idx, args, device, run_dir):
    ...
    if args.ae_checkpoint_dir:
        ...
        encoder = load_fold_encoder(
            args.ae_checkpoint_dir, fold_idx, raw_dim, args.latent_dim, device
        )
...
def ensemble_test_eval(args, device, ckpt_paths, encoders):
    ...
    for k, ckpt in enumerate(ckpt_paths, start=1):
        ...
        test_ds = (
            encode_dataset(test_ds_raw, enc, device) if enc is not None else test_ds_raw
        )
    ...
    return classifier_metrics(probs_sum / len(ckpt_paths), targets_ref)
...
def save_cv_run(run_dir, args, per_fold, ensemble_metrics):
    ...
    with open(run_dir / "summary.txt", "w") as f:
        ...
        for fold in per_fold:
            f.write(
                f"fold {fold['fold']}: epochs_run={len(fold['history']['train_losses'])}, "
...
def main():
    ...
    for k in range(1, args.folds + 1):
        history, val_metrics, ckpt_path, encoder = train_single_fold(
            k, args, device, run_dir
        )
