...
def create_parser():
    parser = argparse.ArgumentParser(description="Generate embeddings from FASTA sequences.")
    parser.add_argument("--input-path", required=True, help="Path to input .fasta file.")
    parser.add_argument("--output-path", required=True, help="Directory where results will be saved.")
    parser.add_argument(
        "--pooling", default="mean", choices=["mean", "max"],
        help="Type of pooling: 'mean' or 'max' (default: mean).",
    )
    parser.add_argument("--batch-size", type=int, default=1024, help="Sequences per batch.")
    parser.add_argument(
